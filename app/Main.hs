...
setup window = do
    ...
    let initialGrid = createGrid squaresToClear
    let minedGrid = mineTheField initialGrid randomPairs
    grid <- liftIO $ newIORef minedGrid
    ...
    displayCanvas canvas minedGrid      --Display grid on the screen
    ...
    on UI.click aiMove $ \_ ->
        do currGrid <- liftIO $ readIORef grid --Should only run one AI move at a time
           let newGrid = computerSolve currGrid
    ...
    on UI.click canvas $ \_  -> do
        ...
        case m of
            Uncover -> do
                let newGrid = clearSquare currGrid (row, col)
                liftIO $ writeIORef grid newGrid
                liftIO $ print newGrid
                displayCanvas canvas newGrid
            Flag -> do
                let newGrid = performMoves currGrid [(Action toggleFlag (row, col))]
                liftIO $ writeIORef grid newGrid
                liftIO $ print newGrid
                displayCanvas canvas newGrid
